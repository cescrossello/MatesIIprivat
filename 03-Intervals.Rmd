...
* Si diem que un *interval de confiança del 95%* per a la concentració mitjana d'una proteïna en sèrum en individus sans mesurada en g/dl sigui [11,16], això significa que hem pres una mostra aleatòria de concentracions d'aquesta proteïna en sèrum en individus sans i a partir d'aquesta mostra hem estimat que, amb un 95% de confiança, la concentració mitjana d'aquesta proteïna en sèrum en individus sans està entre 11 i 16 g/dl (i tenim un 95% de confiança en aquest interval perquè l'hem calculat amb una fórmula que dóna un interval que conté la mitjana poblacional un 95% de les vegades que l'empram).

* Si diem que el 95% de les mostres de 100 concentracions d'una determinada proteïna en sèrum en individus sans està entre 11 i 16, això és un *interval de referència del 95% per a la mitjana mostral* de mostres de mida 100, no un interval de confiança per a la concentració mitjana poblacional ni un interval de referència per al valor de la concentració en un individu.
...
Per tant:
$$
P(-t_{n-1,0.975}\leq  T\leq  t_{n-1,0.975})=0.95
$$
...
```{block2, type='rmdimportant'}
**Teorema** Si $X\sim N(\mu,\sigma)$, un IC 95% per a $\mu$ és
$$
...
`r 200-Encerts` errors de 200 és aproximadament el que esperàvem. Si ho provau amb altres llavors d'aleatorietat obtendreu altres resultats, de vegades millors, de vegades pitjors.
...
A partir d'ara, per evitar ambigüitats, a les fórmules expressarem el nivell de confiança dels intervals en tant per u, no en tant per cent, i parlarem d'intervals de confiança de nivell de confiança *q*, amb *q* entre 0 i 1, en lloc d'intervals de confiança del *100q%*. És a dir, per exemple, els intervals de confiança del 95% seran intervals de confiança de nivell de confiança 0.95.
...
La variable aleatòria d'interès és $X$ és "Prenem un cartutxo d'aquesta empresa i miram el nombre de radiografies que permet imprimir", de mitjana $\mu$ per a la qual volem calcular un IC 90%. Suposarem que la variable $X$ és normal, perquè l'empresa ho suposa a les dades tècniques.

L'interval demanat és:
$$
...
[504.35,531.65]
$$ 
No conté el 500 (en benefici del consumidor)
...
Com que $n$=25 no és prou gran, no podríem aplicar la fórmula de l'IC basada en la t de Student. Emprarem el mètode del *bootstrap*, per a la qual cosa necessitam tenir les dades originals, i no només els seus estadístics. Aquestes dades formen  el vector `Radios` següent:
```{r}
...
Obtenim l'interval $[`r round(quantile(Simulacions,0.05),2)`,`r round(quantile(Simulacions,0.95),2)`]$; no està mal, no?
...
\begin{equation}
\text{estimador}\pm \frac{1+q}{2}\text{-quantil de la distribució mostral}\times \text{error típic de la mostra}
(\#eq:ICMgral)
\end{equation}

Aquesta estructura és molt típica (però de vegades és una altra) i satisfà:
...
El **marge d'error** (**error**, **precisió**) $M$ en l'estimació de $\mu$ per mitjà d'aquest IC és la meitat de la seva amplada:
$$
M=t_{n-1,(1+q)/2}\times \frac{\widetilde{S}_X}{\sqrt{n}}
$$


Una pregunta que ens solem fer a l'hora de dissenyar un experiment és quina ha de ser la mida de la mostra que hem de prendre perquè el marge d'error sigui com a màxim un cert valor desitjat $M_{max}$. Fixau-vos que a la fórmula per $M$ que acabam de donar, la $n$ hi apareix al quantil i a l'error típic, i a més la $\widetilde{S}_X$ depèn de la mostra. El que farem per respondre la pregunta serà fer algunes trampes:
...
* Estimarem $\widetilde{S}_X$ mitjançant la desviació típica mostral $\widetilde{S}_0$ d'una **prova pilot** (una experiment anterior, realitzat per nosaltres amb una mostra petita o publicat a qualque treball)
...
Naturalment, quan després prenguem una mostra de mida $n$ calculada d'aquesta manera, pot passar qualsevol cosa, ja que hem emprat els resultats d'una mostra aleatòria per estimar la desviació típica d'una altra mostra i a més hem aproximat els quantils de la t de Student per els d'una normal estàndard. Però almenys haurem fet tot el que haurem pogut per fitar l'error dins el marge desitjat.
...
El valor de $n$ més petit que satisfà aquesta condició és 68, per tant aquest és el nombre mínim de persones que hauríem hagut d'emprar per obtenir un IC 95% amb un error de (com màxim) 0.5.
...
Per fer-ho, prenem una mostra aleatòria simple de $X$ de mida $n$,amb nombre d'èxits $x$ i per tant freqüència relativa d'èxits $\widehat{p}_{X}=x/n$
...
* El **mètode aproximat de Laplace**, que és una aproximació senzilla del mètode de Wilson, però només es pot emprar quan la mostra és bastant més gran, posem de mida 100 o més, i la proporció mostral $\widehat{p}_{X}$ no és molt propera a 0 o a 1. És el mètode més clàssic i conegut.
...
L'interval de Clopper-Pearson té l'inconvenient que,en general, no està centrat en $\widehat{p}_{X}$. Per exemple, el centre de l'interval anterior és $(`r binom.exact(2,10,0.95)$lower`+`r binom.exact(2,10,0.95)$upper`)/2=	`r (binom.exact(2,10,0.95)$lower+ binom.exact(2,10,0.95)$upper)/2`$, diferent de $\widehat{p}_X=0.2$
...
$$
\widehat{p}_{X}\pm z_{\frac{q+1}{2}}\sqrt{\frac{\widehat{p}_{X}
(1-\widehat{p}_{X})}{n}}
$$
```
...
(1-\widehat{p}_{X})}{n}}
$$
amb $n=500$, $\widehat{p}_{X}=340/500=0.68$ i $z_{\frac{q+1}{2}}=z_{0.975}=1.96$. 
Dóna
$$
...
* L'interval de Clopper-Pearson és exacte, no empra cap aproximació, però:
    ...
    * No té la forma "proporció mostral $\pm$ error"
    ...
    * Com que fins fa poc no es podia calcular, encara no és molt popular

* L'interval de Wilson és aproximat, fa servir l'aproximació a la normal donada pel Teorema Central del Límit. Això segurament no sigui un gran què, pequè tanmateix segurament la mostra serà "aproximadament" aleatòria simple. Ara bé, tampoc no té la forma "proporció mostral $\pm$ error"

* L'interval de Laplace és *molt* aproximat, però
    * Forma part de la cultura general del científic, tothom el coneix
    * És l'únic que té la forma "proporció mostral $\pm$ error"
...
i per tant
$$
M=z_{(q+1)/2} \sqrt{\frac{\widehat{p}_{X} (1-\widehat{p}_{X})}{n}}\leq z_{(q+1)/2}\sqrt{\frac{0.5^2}{n}}=\frac{0.5z_{(q+1)/2}}{\sqrt{n}}=\frac{z_{(q+1)/2}}{2\sqrt{n}}
$$
...
\right],
$$
on $\chi_{\nu,r}^2$ és el $r$-quantil de la distribució $\chi_{\nu}^2$
```

```{block2, type='rmdcorbes'}
La justificació d'aquesta fórmula és la usual: com que 
$$
P(\chi_{n-1}^2\leq \chi_{n-1,(1-q)/2}^2)=\frac{1-q}{2},\quad  
P(\chi_{n-1}^2\geq \chi_{n-1,(1+q)/2}^2)=\frac{1-q}{2},
$$
tenim que
$$
\begin{array}{l}
...
\end{array}
$$
Ara $\chi_{n-1}^2$ no és simètrica, així que s'han de calcular $\chi_{n-1,(1-q)/2}^2$ i $\chi_{n-1,(1+q)/2}^2$. 
````

Fixau-vos que aquest interval de confiança per $\sigma^2$ no està centrat en $\widetilde{S}_{X}^2$.
...
Volem calcular un interval de confiança per a la desviació típica $\sigma$ d'aquest temps d'actuació, amb nivell de confiança  95%. Sabem la fórmula per a l'IC del 95% per a $\sigma^2$:
$$
\left[ \frac{(n-1)\widetilde{S}_{X}^2}{\chi_{n-1,(1+q)/2}^2},
\frac{(n-1)\widetilde{S}_{X}^2}{\chi_{n-1,(1-q)/2}^2}
\right]
...
*Aquest  interval és per a la variància!* Per obtenir un IC per a la desviació típica, prenem arrels quadrades dels extrems:
$$
...
* Si $X$ és una població  de mida $N$ amb mitjana poblacional $\mu$ desconeguda i prenem una m.a.\ sense reposició de $X$, amb mitjana $\overline{X}$ i desviació típica mostral $\widetilde{S}_X$, i si $X$ normal o si $n$ és  gran, es recomana prendre com a IC de nivell de confiança *q* per a $\mu$
$$
\overline{X}\pm t_{n,(q+1)/2}\frac{\widetilde{S}_X}{\sqrt{n}}\sqrt{\frac{N-n}{N-1}}
$$


* Si $X$ una població de mida $N$ que segueix una distribució Bernoulli amb $p_X$ desconeguda i prenem una m.a. sense reposició de $X$, amb $n$ molt gran i nombres d'èxits i fracassos com a mínim 10, es recomana prendre com a IC de nivell de confiança *q* per a $p_X$
$$
\widehat{p}_{X}\pm z_{(q+1)/2}\sqrt{\frac{\widehat{p}_{X}
(1-\widehat{p}_{X})}{n}}\sqrt{\frac{N-n}{N-1}}
$$
