...
Naturalment, no cal saber aquesta fórmula. El que cal saber, d'entrada, és que:
...
```{r, echo=FALSE, fig.width=4, fig.asp = 0.5}
...
* Si $X$ és una variable aleatòria binomial *B(n,p)*, amb *n* gran i *p* lluny de 0 i 1 (prenguem, com a conveni, si $n\geq 100$, $10\leq pn\leq n-10$, però com més centrada sigui *p*, més petit pot ser *n*), alehores $X$ és aproximadament $N(np,\sqrt{np(1-p)})$, en el sentit que les dues funcions de densitat (salvant la diferència pel fet que la binomial és discreta i la normal contínua) són semblants:

```{r,echo=FALSE}
y=0:30
plot(y,dbinom(y,30,0.47),pch=20,xlab="",ylab="", col="red",
     main="B(30,0.47) i N(30*0.47,sqrt(30*0.47*(1-0.47)))")
curve(dnorm(x,30*0.47,sqrt(30*0.47*0.53)),col="blue",add=TRUE,lwd=2)
legend("topright",legend=c("Normal","Binomial"),col=c("blue","red"),
       pch=c(NA,20), lty=c("solid",NA),lwd=c(2,1),cex=0.75)
```


* Si $X$ és una variable aleatòria de Poisson $Po(\lambda)$ i $\lambda$ és gran (posem, $\lambda\geq 50$), aleshores $X$ és aproximadament $N(\lambda,\sqrt{\lambda})$
...
Si $X\sim N(\mu,\sigma)$, la seva densitat $f_X$ és simètrica respecte de $x=\mu$,
$$
f_{X}(\mu-x)=f_{X}(\mu+x),
$$
i té el màxim en $x=\mu$: diem aleshores que $\mu$ és la **moda** de $X$. (Recordem que no té sentit definir la moda d'una variable contínua $X$ com el valor $x_0$ tal que $P(X=x_0)$ sigui màxim, perquè $P(X=x)=0$ per a tot $x\in \mathbb{R}$. Es defineix llavors la **moda** d'una variable contínua $X$ com el valor $x_0$ tal que $f_X(x_0)$ és màxim.)
...
Recordem que la funció de distribució $F_X(x)=P(X\leq x)$ és l'àrea compresa entre la corba definida per la densitat $y=f_X(x)$ i l'eix d'abscisses a l'esquerra de $x$.
...
Indicarem amb $z_q$ el $q$**-quantil** d'una variable normal estàndard $Z$. És a dir, $z_q$ és el valor tal que $P(Z\leq z_q)=q$. A banda del fet que $z_{0.5}=0$ (la mediana de $Z$ és 0),  hi ha dos quantils més de la normal estándard que heu de saber diguem-ne "de memòria":

* $z_{0.95}=1.64$

* $z_{0.975}=1.96 (\approx 2)$

Això significa que $P(Z<-1.64)=P(Z>1.64)=0.05$ i $P(Z<-1.96)=P(Z>1.96)=0.025$.

## Amb R

Per calcular probabilitats d'una $N(\mu,\sigma)$ emprant R, heu de recordar que la normal és `norm`.
...
* Què val el primer quartil de $X\sim N(3,0.5)$?
...
El resultat següent descriu el comportament de l'esperança i la variància d'una combinació lineal de variables aleatòries:

```{theorem, combvar}
Siguin $Y_1,\ldots,Y_n$ variables aleatòries, $a_1,\ldots,a_n,b\in \mathbb{R}$, i $Y=Y=a_1Y_1+\cdots+a_nY_n+b$. Aleshores

1. $E(Y)=a_1E(Y_1)+\cdots+a_nE(Y_n)+b$.

2. Si $Y_1,\ldots,Y_n$ són **independents**, aleshores $\sigma_Y^2=a_1^2\sigma_1^2+\cdots+a_n^2\sigma_n^2$ i per tant $\sigma_Y=\sqrt{a_1^2\sigma_1^2+\cdots+a_n^2\sigma_n^2}$.
...
* $\sigma_Y^2=a_1^2\sigma_1^2+\cdots+a_n^2\sigma_n^2$ i per tant $\sigma=\sqrt{a_1^2\sigma_1^2+\cdots+a_n^2\sigma_n^2}$
```
...
Quan $X\sim N(\mu,\sigma)$, aquests intervals de referència es prenen *sempre centrats en la mitjana* $\mu$, és a dir, de la forma $(\mu-x,\mu+x)$. Per calcular-los, podem emprar el resultat següent:


```{theorem} 
Si $X\sim N(\mu,\sigma)$,
$$
P\big(\mu- z_{(1+q)/2}\cdot \sigma\leq X\leq \mu+ z_{(1+q)/2}\cdot \sigma\big)=q
$$
on $z_{(1+q)/2}$ indica el $(1+q)/2$-quantil de $Z\sim N(0,1)$.
...
En particular, com que si $q=0.95$, aleshores $(1+q)/2=0.975$ i llavors $z_{0.975}=1.96\approx 2$, l'interval de referència del 95% per a $X\sim N(\mu,\sigma)$ és
$$
\mu\pm 1.96\sigma
$$
o simplement $\mu\pm 2\sigma$, per simplificar. Això diu, bàsicament, que si una població segueix una distribució normal $N(\mu,\sigma)$, un 95% dels seus individus estan a distància como a màxim $2\sigma$ ("a dues sigmes") de $\mu$.
...
És l'interval [`r round(163.1+qnorm(0.75)*18.53*c(-1,1),1)`]. Per tant, la meditat de les dones europees de 18 anys fan entre 
`r round((163.1-qnorm(0.75)*18.53)/100,2)` m i `r round((163.1+qnorm(0.75)*18.53)/100,2)` m.
...
$$
163.1\pm 1.96\times 18.53\Longrightarrow [`r round(163.1+qnorm(0.975)*18.53*c(-1,1),1)`].
$$


## El z-score 

El **z-score** (**valor z**, **puntuació z**) d'un valor $x_0$ respecte d'una distribució $N(\mu,\sigma)$ és 
$$
\frac{x_0-\mu}{\sigma}
$$
Com més gran és en valor absolut, més "rar" és $x_0$; el signe ens diu si està per damunt o per davall del valor esperat. 
